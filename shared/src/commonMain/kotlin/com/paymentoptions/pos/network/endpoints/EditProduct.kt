...
suspend fun editProduct(
    ...
    platformLog(TAG, "editProduct: $productId, $request, image provided: ${imageBytes != null}")

    val responseHttp = KtorClient.instance.put(ConfigurationManager.url(ApiEndpoints.EDIT_PRODUCT + productId)) {
        contentType(ContentType.Application.Json)
        applyDaspayHeaders()
        setBody(request)
    }
    responseHttp.throwIfNotSuccess("editProduct")
    ...
    if (imageBytes != null && (response.statusCode == 200L || response.statusCode == 201L)) {
        ...
        if (!signedUrl.isNullOrBlank()) {
            try {
                ...
                platformLog(TAG, "Image updated successfully for product: $productId")
            } catch (e: Exception) {
                platformLog(TAG, "Failed to update image: ${e.message}")
            }
        }
    }

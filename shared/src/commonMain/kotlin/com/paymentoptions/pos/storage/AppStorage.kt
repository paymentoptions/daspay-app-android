 ...
 *
 * Backed by EncryptedSharedPreferences on Android and NSUserDefaults on iOS.
 * Call [AppStorage.init] once at app start before any read/write.
 */
object AppStorage {

    private lateinit var settings: Settings

    /** Called from MainActivity (Android) or MainViewController (iOS) at startup. */
    ...
    // ── Full auth details response (JSON blob) ────────────────────────────────
    ...
    // ── Device / external configuration (JSON blob) ───────────────────────────
    ...
    // ── Biometric / immersive preferences ────────────────────────────────────

    var isBiometricEnabled: Boolean
        get() = settings[KEY_BIOMETRIC_ENABLED, false]
        set(v) { settings[KEY_BIOMETRIC_ENABLED] = v }

    // ── Generic key-value (for arbitrary preferences) ─────────────────────────

    fun getString(key: String): String? = settings.getStringOrNull(key)
    fun putString(key: String, value: String) { settings[key] = value }
    fun getBoolean(key: String, default: Boolean = false): Boolean = settings[key, default]
    fun putBoolean(key: String, value: Boolean) { settings[key] = value }
    ...
    fun clearAuthData() {
        platformLog("AppStorage", "Clearing auth data")
        accessToken = null
        idToken = null
        refreshToken = null
        tokenExpiry = 0L
